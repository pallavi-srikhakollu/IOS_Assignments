...
- (IBAction)saveFriendsDetalis:(id)sender {
    UIAlertView *alertView =[[UIAlertView alloc]initWithTitle:@"error message" message:nil delegate:self cancelButtonTitle:@"ok" otherButtonTitles: nil];
    if (textFieldFirstName.text.length == 0) {
        alertView.message = @"Please enter first name";
        [alertView show];
    }
    else if (textFieldLastName.text.length == 0) {
        alertView.message = @"Please enter last name";
        [alertView show];
    }
        ...
        alertView.message = @"Please enter state";
        [alertView show];
    }
